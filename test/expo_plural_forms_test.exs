defmodule ExpoPluralFormsTest do
  @moduledoc false

  use ExUnit.Case, async: true

  doctest ExpoPluralForms

  describe "parse/1" do
    test "Only one form" do
      assert {:ok, {1, 0}} = ExpoPluralForms.parse_string("nplurals=1; plural=0;")
    end

    test "Two forms, singular used for one only" do
      assert {:ok, {2, {:!=, :n, 1}}} = ExpoPluralForms.parse_string("nplurals=2; plural=n != 1;")
    end
    ...
    test "invalid expression" do
      assert {:error, {:parse_error, _message, 1, 19}} =
               ExpoPluralForms.parse_string("nplurals=6; plural=m % 20 == 1;")
    end
  end
  ...
  describe "index/2" do
    ...
    test "identity" do
      assert 2 = ExpoPluralForms.index(:n, 2)
      assert 7 = ExpoPluralForms.index(7, 2)
    end

    test "operators" do
      assert 0 = ExpoPluralForms.index({:%, 10, 2}, 2)
      assert 1 = ExpoPluralForms.index({:%, 10, 3}, 2)

      assert 0 = ExpoPluralForms.index({:>, 1, 2}, 2)
      assert 1 = ExpoPluralForms.index({:>, 2, 1}, 2)
      assert 0 = ExpoPluralForms.index({:>, 1, 1}, 2)

      assert 0 = ExpoPluralForms.index({:>=, 1, 2}, 2)
      assert 1 = ExpoPluralForms.index({:>=, 2, 1}, 2)
      assert 1 = ExpoPluralForms.index({:>=, 1, 1}, 2)

      assert 1 = ExpoPluralForms.index({:<, 1, 2}, 2)
      assert 0 = ExpoPluralForms.index({:<, 2, 1}, 2)
      assert 0 = ExpoPluralForms.index({:<, 1, 1}, 2)
      ...
      assert 1 = ExpoPluralForms.index({:&&, 1, 1}, 2)
      assert 0 = ExpoPluralForms.index({:&&, 0, 1}, 2)
      assert 0 = ExpoPluralForms.index({:&&, 1, 0}, 2)
      assert 0 = ExpoPluralForms.index({:&&, 0, 0}, 2)

      assert 1 = ExpoPluralForms.index({:||, 1, 1}, 2)
      assert 1 = ExpoPluralForms.index({:||, 0, 1}, 2)
      assert 1 = ExpoPluralForms.index({:||, 1, 0}, 2)
      assert 0 = ExpoPluralForms.index({:||, 0, 0}, 2)
    end
    ...
    test "conditional" do
      ...
      assert 7 = ExpoPluralForms.index(condition, 3)
      assert 0 = ExpoPluralForms.index(condition, 0)
    end
  end
  ...
  test "arabic" do
    ...
    assert ExpoPluralForms.index(plurals, 0) == 0
    assert ExpoPluralForms.index(plurals, 1) == 1
    assert ExpoPluralForms.index(plurals, 2) == 2
    assert ExpoPluralForms.index(plurals, 2009) == 3
    assert ExpoPluralForms.index(plurals, 2011) == 4
    assert ExpoPluralForms.index(plurals, 2099) == 4
    assert ExpoPluralForms.index(plurals, 3000) == 5
  end
  ...
  describe "plural_form/1" do
    ...
    test "unknown locale" do
      assert :error = ExpoPluralForms.plural_form("wat")

      # This happens with dash as the territory/locale separator
      # (https://en.wikipedia.org/wiki/IETF_language_tag).
      assert :error = ExpoPluralForms.plural_form("en-us")
    end
  ...
  defp plural(code, n) do
    {:ok, {_nplurals, plural_form}} = ExpoPluralForms.plural_form(code)
    ExpoPluralForms.index(plural_form, n)
  end

  defp nplurals(code) do
    {:ok, {nplurals, _plural_form}} = ExpoPluralForms.plural_form(code)
    nplurals
  end
  ...
  describe "compose/1" do
    test "converts plural form back to string" do
      ...
      assert "nplurals=6; plural=(n==0 ? 0 : n==1 ? 1 : n==2 ? 2 : n%100>=3 && n%100<=10 ? 3 : n%100>=11 ? 4 : 5);" =
               plural_forms |> ExpoPluralForms.compose() |> IO.iodata_to_binary()
    end

    for locale <- ExpoPluralForms.known_locales() do
      {:ok, plural_form} = ExpoPluralForms.plural_form(locale)
  ...
  describe "compile_index/1" do
    test "simple" do
      assert {:ok, {2, plurals}} = ExpoPluralForms.parse_string("nplurals=2; plural=n>1;")

      quoted = ExpoPluralForms.compile_index(plurals)
    ...
    test "one form does not error" do
      assert {:ok, {2, plurals}} = ExpoPluralForms.parse_string("nplurals=2; plural=0;")
  ...
  describe "known_locales/0" do
    assert "en" in ExpoPluralForms.known_locales()
  end

  {:ok, {2, plurals}} = ExpoPluralForms.parse_string("nplurals=2; plural=n>1;")
  defp index_simple(n), do: unquote(ExpoPluralForms.compile_index(plurals))
end
